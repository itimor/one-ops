...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        ops_tl = User.objects.create(username='ops_tl', password=make_password("123456"), realname="小王", group=group_ops)
        ops_tl.roles.add(role_ops_tl)
        ops = User.objects.create(username='ops', password=make_password("123456"), realname="小基", group=group_ops)
        ops.roles.add(role_ops)
        dev_tl = User.objects.create(username='dev_tl', password=make_password("123456"), realname="小强", group=group_dev)
        dev_tl.roles.add(role_dev_tl)
        dev = User.objects.create(username='dev', password=make_password("123456"), realname="小虎", group=group_dev)
        dev.roles.add(role_dev)
        hr_tl = User.objects.create(username='hr_tl', password=make_password("123456"), realname="奔波儿灞", group=group_hr)
        hr_tl.roles.add(role_hr_tl)
        hr = User.objects.create(username='hr', password=make_password("123456"), realname="霸波儿奔", group=group_hr)
